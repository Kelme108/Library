...
const Livro = props => (
    <tr>
        ...
        <td>
            ...
            <Button className={estilo.btn} size="sm" variant="danger" onClick={() => props.apagarLivro(props.livro.id)}>Excluir</Button>
            <Button className={estilo.btn} href={"/editar-livro/" + props.livro.id} size="sm" variant="info" onClick={(e) => e.preventDefault()}>
                <Link to={"/editar-livro/" + props.livro.id} >editar</Link>
            </Button>
        </td>
    </tr>
)
...
class ListagemLivros extends Component {
    ...
    componentDidMount() {
        ...
        Axios.get('http://localhost/livros')
            .then(response => {
                this.setState({
    ...
    apagarLivro(id) {
        Axios.delete('http://localhost/livros/' + id)
        ...
        this.setState({
            ...
            livros: this.state.livros.filter(el => el.id !== id)
        })
